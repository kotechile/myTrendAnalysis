...
def create_simplified_content_opportunities_prompt(topic: str, trending_topics: List[Dict]) -> str:
    """
    Simplified content opportunities prompt
    """
    topics_text = ", ".join([t.get("trend_name", "") for t in trending_topics[:3]])
    
    return f"""
Based on these trending topics: {topics_text}

Create EXACTLY 5 content opportunities as JSON array:

[
  {{
    "opportunity_title": "Complete Guide to [specific topic]",
...
Focus on PRACTICAL content that businesses can create quickly.
Return only valid JSON, no other text.
"""
...
class EnhancedTrendResearchIntegration:
        ...
        try:
            ...
            if PYTRENDS_FIXED_AVAILABLE:
                ...
                try:
                    ...
                    # Extract subtopics from trending topics or use as parameter
                    subtopics = []
                    if 'trending_topics' in data:
                        # Extract subtopics from trending topics data
                        for topic_data in data.get('trending_topics', []):
                            if isinstance(topic_data, dict) and 'trend' in topic_data:
                                subtopics.append(topic_data['trend'])
                    
                    # Use the FIXED PyTrends integration with subtopics
                    ...
                    # Add subtopics to the PyTrends data if available
                    if enhanced_result.get('pytrends_analysis'):
                        enhanced_result['pytrends_analysis']['subtopics_used'] = subtopics
    ...
    async def _get_content_opportunities_fast(
        ...
        """Fast content opportunities analysis"""
        
        # Create simplified prompt
        prompt = create_simplified_content_opportunities_prompt(topic, trending_topics_data)
        ...
        try:
            ...
            # Parse response
            content_opportunities = self._parse_content_opportunities_response(response, topic)
            
            self.logger.info(f"💡 Generated {len(content_opportunities)} content opportunities")
            return content_opportunities
            
        except asyncio.TimeoutError:
            self.logger.error("❌ LLM call timed out for content opportunities")
            return self._fallback_content_opportunities(topic)
        except Exception as e:
            self.logger.error(f"❌ Content opportunities analysis failed: {e}")
            return self._fallback_content_opportunities(topic)
